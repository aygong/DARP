...
class GraphTransformerNet(nn.Module):
    def __init__(self, 
        ...
        self.MLP_value_layer = nn.Sequential(
            nn.Linear(num_nodes * d_model, d_ff), 
            nn.ReLU(),
            nn.Linear(d_ff, d_ff),
            nn.ReLU(),
            nn.Linear(d_ff, 1) 
    ...
    def forward(self, g, h, e, vehicle_node_id, num_nodes, h_lap_pe=None, masking=True):
        # input embedding
    ...
    def predict_action(self, g, h, e, vehicle_node_id, num_nodes, h_lap_pe=None, masking=True):
        h = self.embedding_h(h)
